"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const path_1 = __importDefault(require("path"));
const os_1 = __importDefault(require("os"));
function setPath() {
    // IMPORTANT: needed to run zigbee2mqtt in plugin
    // return process.env.ZIGBEE2MQTT_DATA ? process.env.ZIGBEE2MQTT_DATA : path_1.default.normalize(path_1.default.join(__dirname, '..', '..', 'data'));
    // return process.env.ZIGBEE2MQTT_DATA ? process.env.ZIGBEE2MQTT_DATA : path_1.default.normalize(path_1.default.join(__dirname, '..', '..', '..', 'config'));
    return process.env.ZIGBEE2MQTT_DATA ? process.env.ZIGBEE2MQTT_DATA : path_1.default.normalize(path_1.default.join(os_1.default.homedir(), '.aydo', 'zigbee2mqtt'));
}
let dataPath = setPath();
function joinPath(file) {
    return path_1.default.resolve(dataPath, file);
}
function getPath() {
    return dataPath;
}
function _testReload() {
    dataPath = setPath();
}
exports.default = { joinPath, getPath, _testReload };
//# sourceMappingURL=data:application/json;base64,